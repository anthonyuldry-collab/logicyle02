 ...
 * Aligné sur le modèle missions vacataires (take rate Rovik).
 */
...
export const PARTNERSHIP_MARKETPLACE_LABELS = {
  feeExplanation: {
    fr: 'Commission Rovik : {rate} % sur le montant du partenariat facilité via la plateforme (min. {min} €).',
    en: 'Rovik fee: {rate}% on partnership amount facilitated via the platform (min. €{min}).',
  },
  proDiscount: {
  ...
  howItWorks: {
    fr: 'Les partenaires et les clubs se connectent via Rovik. En cas de signature, une commission de mise en relation s\'applique.',
    en: 'Partners and clubs connect via Rovik. Upon signing, a matchmaking fee applies.',
  },
} as const;
