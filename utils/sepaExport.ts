...
/** Génère le XML pain.008 (utilisable en tests). */
export function generateSepaPain008XmlContent(
  teamName: string,
  settings: TeamSepaSettings,
  ...
  const readyOrders = orders.filter(
    (o) =>
      o.amount > 0 &&
      (o.isExportReady === true ||
        (o.isExportReady === undefined && o.hasValidIban && o.hasValidBic !== false && o.hasValidMandate !== false))
  );
  if (readyOrders.length === 0) return null;
  if (!settings.creditorIdentifier?.trim()) return null;
  ...
  const ics = settings.creditorIdentifier.trim();
  const normalizedOrders = normalizeCollectionSequencesForBatch(readyOrders);

  const bySeq = new Map<string, SepaCollectionOrder[]>();
  for (const order of normalizedOrders) {
    const seq = order.mandateSequence || 'OOFF';
    const list = bySeq.get(seq) || [];
    list.push(order);
    bySeq.set(seq, list);
  }

  const paymentInfos = sortSeqTypes(bySeq.keys())
    .map((seqTp, idx) => {
      const seqOrders = bySeq.get(seqTp)!;
      const seqSum = seqOrders.reduce((sum, o) => sum + o.amount, 0).toFixed(2);
      const transactions = seqOrders
        .map((order) => {
          const debtorBic = order.beneficiaryBic?.trim().toUpperCase() || '';
          const signDate = order.mandateSignedAt || execDate;
          return `
      <DrctDbtTxInf>
        ...
        <DrctDbtTx>
          <MndtRltdInf>
            <MndtId>${escapeXml((order.mandateReference || order.incomeItemId).slice(0, 35))}</MndtId>
            <DtOfSgntr>${escapeXml(signDate)}</DtOfSgntr>
          </MndtRltdInf>
        </DrctDbtTx>
        <DbtrAgt>
          <FinInstnId>${debtorBic ? `<BIC>${escapeXml(debtorBic)}</BIC>` : '<Othr><Id>NOTPROVIDED</Id></Othr>'}</FinInstnId>
      ...
      return `
    <PmtInf>
      <PmtInfId>${escapeXml(msgId)}-${idx + 1}</PmtInfId>
      <PmtMtd>DD</PmtMtd>
      <NbOfTxs>${seqOrders.length}</NbOfTxs>
      <CtrlSum>${seqSum}</CtrlSum>
      <PmtTpInf><SvcLvl><Cd>SEPA</Cd></SvcLvl><LclInstrm><Cd>CORE</Cd></LclInstrm><SeqTp>${escapeXml(seqTp)}</SeqTp></PmtTpInf>
      ...
      <ChrgBr>SLEV</ChrgBr>
      <CdtrSchmeId><Id><PrvtId><Othr><Id>${escapeXml(ics)}</Id><SchmeNm><Prtry>SEPA</Prtry></SchmeNm></Othr></PrvtId></Id></CdtrSchmeId>
      ${transactions}
    </PmtInf>`;
    })
    .join('');
...
<Document xmlns="urn:iso:std:iso:20022:tech:xsd:pain.008.001.02">
  <CstmrDrctDbtInitn>
    <GrpHdr>
      <MsgId>${escapeXml(msgId)}</MsgId>
      <CreDtTm>${now.toISOString()}</CreDtTm>
      <NbOfTxs>${normalizedOrders.length}</NbOfTxs>
      <CtrlSum>${normalizedOrders.reduce((sum, o) => sum + o.amount, 0).toFixed(2)}</CtrlSum>
...
export function exportSepaPain008Xml(
  ...
  const xml = generateSepaPain008XmlContent(teamName, settings, orders, executionDate);
  if (!xml) return;
